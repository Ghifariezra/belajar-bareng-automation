...
- `canvas` and `pixelmatch` for image-processing and visual comparison support
...
│   ├── locators/
│   │   ├── index.locators.js      # Locator barrel export
│   │   ├── login.locator.js       # Login page locators
│   │   ├── dashboard.locator.js   # Dashboard and Add User locators
│   │   └── checkout.locator.js    # Checkout flow locators
│   └── pages/
...
│       ├── helpers/
│       │   └── test.helper.js      # Shared after-test screenshot handling
│       ├── modules/
...
├── reports/
│   ├── chrome-report.html         # Generated by the Chrome command
│   ├── firefox-report.html        # Generated by the Firefox command
│   ├── edge-report.html           # Generated by the Edge command
│   ├── compatibility-report.html  # Generated by the parallel command
...
The individual report files are `reports/chrome-report.html`, `reports/firefox-report.html`, and `reports/edge-report.html`. These files are created or overwritten when their respective commands run.
...
The report includes results for Chrome, Firefox, and Microsoft Edge. The individual browser reports and the compatibility report are generated artifacts and may not exist until their corresponding test command has run. Known pending or unstable scenarios are described below.
...
- Locator definitions are separated into `src/locators/` and consumed by the page objects that use the locator layer.
- `tests/belajar-bareng/helpers/test.helper.js` centralizes after-test screenshot handling for passed and failed scenarios.
